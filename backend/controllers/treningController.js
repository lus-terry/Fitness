...
exports.create = async (req, res) => {
  ...
  if (id_trenera !== req.user?.id_korisnika) {
    console.warn('[treningController] ❌ ID trenera iz bodyja NE odgovara prijavljenom korisniku');
  ...
  try {
    ...
    console.error('[treningController] ❌ Greška prilikom unosa treninga:', err.message);
    res.status(500).json({ error: err.message });
  }
};
